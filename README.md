...
| 17 | Every published claim scoped to a pinned environment tuple, and every artifact produced at a clean committed revision | The tuple named the certifier, not the engine. Six artifacts backing an upstream issue printed `torch 2.6.0 / cu12.4` beside a claim about a server running `torch 2.11.0 / cu13.0` in another virtual environment, and that line is what scopes the claim. `scripts/audit_artifacts.py` checked it and passed, because it verified that all artifacts agreed on one tuple rather than that the tuple described the thing being measured. Clean-revision discipline was applied to the harness, with a dedicated artifact pinning its SHA, while the subject was a released wheel to which no revision discipline was applied at all, so nobody asked whether main had moved. A finding was filed upstream against a version whose fix was already merged. This is the shape rows 14 and 16 share, and `.get("killed", 0)` before them: **a consistency check between two things you produced is not a check against the thing they describe.** **Found by an outside reproducer proposing the root cause, which forced the question of what version the subject actually was** |
